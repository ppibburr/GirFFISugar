=begin
  *
    ...
    Only applys to subclasses of library classes
    ie, Gtk::Window is a subclass of GObject, but it will not implement these features
        however, class MyClass < Gtk::Window; ... ; end, will implement these features
                 class Foo < MyClass, would inherit implementation
  *
=end
...
module GObject
  class Object
    ...
    def self.gir_methods cls
      store = {}
      cls.ancestors.find_all do |a|
	a.ancestors.index(GObject::Object)
      end.each do |a|
	a.class_eval do
	  @setup ||= []
	end
	store[a] = a.instance_methods - a.superclass.instance_methods
      end
      {cls=>store[cls]}
    end

    def self.setup_methods store
      store.each_pair do |k,v|
	k.class_eval do
	  v.each do |m|
	    ary = k.instance_variable_get(:@setup) 
	    p [m,k]  if m == :get_children and ary.index(m)
	    next if ary.index(m)
	    
	    ary << m
	    _setup_instance_method(m.to_s)
	  end
	end
      end
    end
  ...
  module Quux
    ...
    def initialize *o
      gtype = Quux.get_gtype self.class
      obj = gtype.new(*o)
    ...
    # get the gtype of the farthest ancestpr implementing Quux
    def self.get_gtype cls
      r = cls.ancestors.find_all do |a|
	a.ancestors.index(GObject::Quux)
      end
      r.pop
      r.last.gtype
    end
